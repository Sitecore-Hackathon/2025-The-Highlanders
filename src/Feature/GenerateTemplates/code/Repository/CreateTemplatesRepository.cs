...
namespace Highlanders.Feature.GenerateTemplates.Repository
{
    ...
    public class CreateTemplatesRepository : ICreateTemplatesRepository
    {
        ...
        public void CreateYmlFiles(string requirement)
        {
            ...
            var appDataPathYmls = HttpUtility.UrlDecode(System.Web.Hosting.HostingEnvironment.MapPath("~/App_Data/ymls"));
            appDataPathYmls += "\\";

            foreach (var file in ymlFilesContainer.YmlFiles)
            {
                string directoryPath = Path.GetDirectoryName(file.Path);
                if (!Directory.Exists(appDataPathYmls + directoryPath))
                {
                    Directory.CreateDirectory(appDataPathYmls + directoryPath);
                }

                File.WriteAllText(appDataPathYmls + file.Path, file.Content);
            }
